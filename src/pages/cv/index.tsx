...
export default function CvPage() {
  ...
  const addAward = () => setAwards((prev) => [...prev, { title: "" } as AwardItem]);
  const updateAward = (index: number, patch: Partial<AwardItem>) =>
  ...
  const addLanguage = () => setLanguages((prev) => [...prev, { name: "" } as LanguageItem]);
  ...
  const onSave = async () => {
    const next = {
      ...draft,
      digitalCard: {
        // Preserve fields owned by the Digital Card and Portfolio pages
        // (templateId, headerColor, paletteStyle, backgroundColor, etc.) —
        // this page only ever overrides the fields below.
        ...draft.digitalCard,
    ...
    if (saveProfile.fulfilled.match(res)) {
      dispatch(pushToast("CV saved", "success"));
    } else {
  ...
  const onDownload = async () => {
    ...
    setDownloading(true);
    try {
      const pages = Array.from(container.querySelectorAll<HTMLElement>("[data-cv-page]"));
      const nameSlug = (draft.personal?.fullName || "resume").trim().toLowerCase().replace(/\s+/g, "-");
      await exportPagesToPdf(pages.length ? pages : [container], `${nameSlug}-cv.pdf`);
      dispatch(pushToast("CV downloaded", "success"));
    } catch {
  ...
  return (
    <AppShell fullHeight>
      ...
      {/* Off-screen, always-mounted full-size copy used only for PDF export —
          ...
          of the full CV, so downloading can't rely on that DOM being present. */}
      <div ref={exportRef} aria-hidden style={{ position: "fixed", top: 0, left: -99999, width: 820 }}>
        {stage}
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-4 lg:shrink-0">
        <div className="min-w-0">
          ...
          <p className="text-sm text-ink/55 dark:text-white/55">
            Build your professional resume and share it as a live link.
          </p>
        </div>
      ...
      <div className="flex min-w-0 flex-col gap-5 lg:min-h-0 lg:flex-1 lg:flex-row">
        ...
        <aside className="rounded-3xl border border-ink/5 bg-mist dark:border-white/5 dark:bg-[#12403c] no-scrollbar lg:w-[380px] lg:h-full lg:shrink-0 lg:overflow-y-auto xl:w-[440px]">
          <div className="px-5 py-4">
            <h3 className="font-display text-lg font-black text-ink dark:text-white">Create CV</h3>
          </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="max-w-[110px] truncate text-xs font-semibold text-ink/60 dark:text-white/60">
                      {CV_TEMPLATES.find((t) => t.id === templateId)?.name || "None selected"}
                    </span>
              ...
              <div className="grid grid-cols-2 gap-3 px-5 pb-4">
                {CV_TEMPLATES.map((tpl) => {
                  ...
                  return (
                    ...
                    >
                      {/* Mini page thumbnail */}
                      <span className="relative block h-20 w-full overflow-hidden rounded-lg">
                        <span className="absolute inset-0" style={{ background: tpl.colors.background }} />
                        <span
                          className="absolute left-2 right-2 top-2 h-1.5 rounded-sm"
                          style={{ background: tpl.colors.heading }}
                        />
                        {tpl.layout.contactBar && (
                          <span
                            className="absolute left-0 right-0 top-6 h-2"
                            style={{ background: tpl.colors.accentBar }}
                          />
                        )}
                        <span className="absolute bottom-2 left-2 right-2 flex gap-1">
                          {Array.from({ length: Math.min(tpl.layout.sectionColumns.skills, 5) }).map((_, i) => (
                            <span
                              ...
                              style={{ background: `${tpl.colors.text}22` }}
                            />
                          ))}
                        </span>
                        {active && (
          ...
          {detailView === "details" && (
            <div className="space-y-1">
              ...
              <div className="space-y-6 px-5 pb-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-black uppercase tracking-wider text-ink/60 dark:text-white/60">Awards</p>
                    <button
                      type="button"
                      onClick={addAward}
                      className="flex items-center gap-1 text-xs font-bold text-brand-600 dark:text-white/80"
                  ...
                  {awards.length === 0 && (
                    <p className="text-xs text-ink/40 dark:text-white/40">No awards added yet.</p>
                  )}
                  <div className="space-y-3">
                    {awards.map((a, i) => (
                      <div key={i} className="space-y-2 rounded-xl border border-ink/10 p-3 dark:border-white/10">
                        <div className="flex items-center gap-2">
                          <input
                            type="text"
                            value={a.title || ""}
                            onChange={(ev) => updateAward(i, { title: ev.target.value })}
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-black uppercase tracking-wider text-ink/60 dark:text-white/60">Languages</p>
                    <button
                      type="button"
                      onClick={addLanguage}
                      className="flex items-center gap-1 text-xs font-bold text-brand-600 dark:text-white/80"
                  ...
                  {languages.length === 0 && (
                    <p className="text-xs text-ink/40 dark:text-white/40">No languages added yet.</p>
                  )}
                  <div className="space-y-3">
                    {languages.map((l, i) => (
                      <div key={i} className="space-y-2 rounded-xl border border-ink/10 p-3 dark:border-white/10">
                        <div className="flex items-center gap-2">
                          <input
                            type="text"
                            value={l.name || ""}
                            onChange={(ev) => updateLanguage(i, { name: ev.target.value })}
                        ...
                        <input
                          type="text"
                          value={l.level || ""}
                          onChange={(ev) => updateLanguage(i, { level: ev.target.value })}
